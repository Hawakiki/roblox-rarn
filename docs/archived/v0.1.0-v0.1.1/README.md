...
> **정정 (2026-08-22).** 위 M17 줄은 RN-6 이 0.1.1 에 실려 나간 것처럼 읽히는데, 그렇지
> 않다. 고침 커밋은 태그 **뒤**에 있고 `git merge-base --is-ancestor` 로 확인된다. M17
> 기록의 본문은 이 순서를 정확히 적어 두었다 — *"0.1.1 을 냈다. 그다음 벤치마크를 만들다
> 여섯 번째(RN-6)를 찾아 함께 고쳤다."* 틀린 것은 이 한 줄 요약이다.
>
> 동결된 문서라 원문은 고치지 않고 정정만 덧붙인다. 현재 상태는
> [`docs/known-issues.md`](../../known-issues.md) 가 갖는다.

...
> **정정 (2026-09-25).** `cache verify` 는 `downloads/` 를 해싱해 `extracted/` 와 대조한 적이
> 없다. 처음 구현(97425a1)부터 보는 것은 두 가지다: zip 과 풀린 트리가 서로 짝이 있는지(존재만),
> 그리고 프로젝트 안이면 각 zip 의 해시가 rarn.lock 의 다이제스트와 같은지. 풀린 트리의 내용은
> 어느 쪽에서도 해싱하지 않으므로 `extracted/` 안에서 고친 파일은 통과한다. 설치도 같은 경계를
> 긋고, 출력은 지금 그 경계대로 "archives verified" 라고 말한다 (#53).
>

...
| | rarn | wally |
|---|---:|---:|
| 설치, 캐시 있음 | **5.6 s** — 네트워크 안 씀 | 9.8 s — 575개 전부 다시 받음 |
| 설치, 캐시 없음 | 37.3 s | 9.8 s |
| 이 머신에서 처음 | 37.3 s | 9.8 s **+ 12 s** 인덱스 clone (46 MB, `git` 필요) |
| 쓴 파일 | **7,045** | 12,956 |
...
- **콜드 설치는 Wally 가 이긴다.** 레지스트리 인덱스를 미리 clone 해 두니까 해석에 네트워크가
  안 든다. Rarn 은 HTTP 로 물어보고 37.3 s 중 8.9 s 를 거기 쓴다. `git` 없이 돌아가고 46 MB
  clone 을 안 들고 다니는 값이라고 보면 된다.

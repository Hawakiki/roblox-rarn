...
import { ATOMIC_TEMP_SUFFIX } from '../src/util/atomic-write.ts'
...
describe('token file', () => {
  // RARN_AUTH_FILE can put the token inside a project, and publish leaves this suffix
  // out by default. A copy stranded by a kill between the write and the rename is the
  // token as well, and pack's identity check cannot see it, being another file.
  test('the temporary copy carries the suffix publish never packs', async () => {
    const rename = spyOn(fsp, 'rename')
    try {
      await writeToken(API, 'gho_first', path)
      expect(rename).toHaveBeenCalledTimes(1)
      expect(String(rename.mock.calls[0]?.[0])).toEndWith(ATOMIC_TEMP_SUFFIX)
    } finally {
      rename.mockRestore()
    }
  })

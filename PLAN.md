...
- [ ] **우리 밖 프로젝트 3~5개가 0.1.x 로 설치해서 돌아간다.**
      지금 별 0, 포크 0, 다운로드는 전부 우리 것이다. Rarn 을 모르는 세션에게 실제
      프로젝트를 만들게 한 것이 한 번 있고([M6](docs/milestones/v1/m06-field-report.md)),
      결함 4개와 문서 3건이 나왔다 — 하지만 **우리가 돌린 세션은 우리다.** 카운트에 안
      들어간다.

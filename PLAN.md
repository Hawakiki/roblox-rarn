...
**0.3.0 이 나갔다 (2026-09-25).** 이슈 #52~#58 을 고친 RN-17~RN-25 가 실렸고, 그중 **RN-22·RN-23
은 0.2.0·0.1.1 의 보안 결함**이다. 락파일 스키마가 조여졌다(`version` 에 semver 패턴) — 위의
스키마 시계가 다시 시작된 이유다. 이 릴리스부터 기본 브랜치는 `master` 다.

**기본 브랜치를 0.3.0 에서 `master` 로 바꿨다.** 저장소 첫 화면이 곧 기본 브랜치인데
`develop` 은 아무도 못 받는 것을 설명한다 — M7 이 릴리스 전에 README 에 올라가 있던 며칠과,
RN-6 이 안 실린 릴리스에 실렸다고 기록돼 있던 것이 같은 병이다. 0.2.0 때 바꾸지 않은 것은
`LICENSE` 가 `develop` 에만 있어서였고, 0.3.0 으로 `master` 에 들어갔다. PR 은 여전히
`develop` 으로 — 기본 브랜치가 더는 그 base 를 대신 골라 주지 않으므로 `ci.yml` 의 `base`
잡이 `master` 로 가는 `feat/*` PR 을 잡는다.

**M4b 만 남았고, 그것은 게이트에 걸려 있다.** 스키마를 건드릴 마지막 후보였던 M7 이 끝났고,
스키마에 닿지 않았다 — shim 내용은 매니페스트도 락파일도 아니다. M8 도 M9 도 닿지 않았다.
게이트의 "마지막 스키마 변경 후 한 달" 시계는 M4a(2026-08-22)부터 돌다가 **2026-09-25 에
다시 시작됐다.** RN-23(고친 락파일의 `version` 이 캐시 밖 디렉터리를 지우던 것)을 막으면서
락파일 스키마의 `version` 에 semver 패턴을 넣었다. 코드에서만 검사했으면 시계는 지켰겠지만,
그건 시계가 재려던 것 — 포맷이 아직 흔들리는가 — 을 가리는 일이라 스키마를 바꿨다. 이 기계의
락파일 14개와 레지스트리 29,281 버전 중 새 규칙이 새로 거절하는 것은 없다.

**게이트 1번 항목의 시계는 2026-09-17 에야 시작됐다.** 2026-08-22 밤 GitHub 계정이 자동 남용
...
플래그는 지원팀의 수동 검토로 2026-09-17 에 풀렸고, 디스코드 첫 공지는 2026-09-20 이다.
...
| **출석부(#47) 에 남이 남긴 댓글** | 핀과 같은 무게의 직접 답이고, 비공개 프로젝트도 잡힌다 |
| **이슈·PR — 우리가 연 것은 뺀다** | 넣었고, 돌렸고, 뭔가 겪었다. **핀 하나보다 무겁다** |
...
**기계적으로 세면 안 된다.** 코드 검색은 공개 저장소만 색인하고 Roblox 게임은 비공개가
많아서 언제나 과소집계된다. 이슈 하나는 핀 여러 개만큼의 값이 있다 — 실제로 돌려봤다는
뜻이기 때문이다. 숫자는 판단의 재료지 판단 자체가 아니다.

**그리고 우리 자신을 세면 안 된다.** `open_issues_count` 를 그대로 읽던 첫 판은 출석부 이슈
하나로 "신호가 있다" 고 말했고, 404 가 풀린 날 처음 돌려 보고서야 드러났다.

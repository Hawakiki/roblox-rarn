...
- [ ] **`docs/known-issues.md` 미해결 0.** **지금 1** — [R3](docs/research/r3-performance.md)
      가 RN-16(대소문자만 다른 alias 가 서로를 덮어쓴다)을 찾았다. 이 항목은 한동안 0
      이었고, 0 이었던 이유는 아무도 안 찾아서가 아니라 찾은 것을 다 고쳤기 때문이다.
      이번엔 성능을 재다가 나왔다.

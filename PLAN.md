...
스키마에 닿지 않았다 — shim 내용은 매니페스트도 락파일도 아니다. M8 도 닿지 않았다.
...
**첫째 것은 착지했다.** 은퇴 트리를 다음 설치가 빌드하면서 지운다. 출하 바이너리로
506패키지 warm 반복 설치 6,352 → 5,057 ms (20.4%, on/off/on). TypeScript 로는 31.9%
가 나오고 **그 격차는 설명되지 않았다** — 바이너리가 사람이 실행하는 쪽이므로 20.4%
가 인용할 숫자다. 상세는 CHANGELOG.

남은 후보들의 상한은 [R3](docs/research/r3-performance.md) §6 에 있고, C2 가 착지한
지금 **다시 재야 한다**. R3 가 warm 경로 구성요소들의 겹침이 57 ms 뿐이라고 측정했으므로
절약이 더해질 것으로 예상되지만, **예상은 측정이 아니다.**

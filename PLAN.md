...
우선순위대로. 1~2 가 끝나기 전에는 아무것도 배포하지 않는다.

1. **RN-1~RN-5 수정** — [docs/known-issues.md](docs/known-issues.md).
   RN-3(dedupe 누락)·RN-5(호환 드리프트)는 **재현부터**. 이 다섯이 R2 §6.4 의
   선행 차단 요소와 같은 집합이다.
2. **0.1.1 재배포** — 릴리스 노트는 영어(언어 정책), `rokit trust` 안내 포함.
3. **REDUCE_SCOPE 나머지 단계** — 크로스 트리 중복 진단, 파이프라인의 cli/ 탈출.
   표는 [research/r2 §6.4](docs/research/r2-workspaces.md).
4. **M15 는 조건부** — 둘 이상의 `rarn.json` 을 한 리포에 둔 실제 사용자가 신호.
   그 전에 여는 것은 R2 가 명시적으로 반대했다.
...
| CI 초록 | ✅ |
| RN-1~RN-5 수정 | ❌ |
| 0.1.1 이 나가고 `rokit add` 동작 | ❌ (0.1.0 은 철회됨) |

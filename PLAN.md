...
| | 마일스톤 | 끝나는 조건 | 왜 동결 전인가 |
|---|---|---|---|
| M1 | 라이브 발행 1회 | 레지스트리에 한 번 발행하고, 그것을 다시 설치해서 돈다 | `publish` 는 `rarn.json` 을 `wally.toml` 로 번역해 올린다. **그 경로가 한 번도 산 적이 없다** |
| M2 | 별칭 충돌 정책 확정 | 정책을 정하고 문서에 적는다. 필요하면 `aliases` 의 의미를 바꾼다 | 매니페스트 필드의 의미가 걸려 있다 |
| M3 | 파이프라인을 `cli/` 에서 꺼내기 | `install` 의 합성이 `src/cli/` 밖에 있고 합성 테스트가 있다 | 동결된 포맷을 지키는 것은 테스트다. 여기가 유일하게 테스트 없는 부분 |
| M4 | 스키마 동결 리뷰 | 두 스키마의 모든 필드를 한 번씩 변호한다 | **이게 동결 그 자체다.** M1·M2 의 결과가 들어온다 |
| M5 | 문서·주장 감사 | README·CLAUDE.md 의 주장을 코드와 대조한다 | 1.0 은 문서도 약속이 된다 |

순서는 M1 · M2 · M3 까지 서로 독립이고, M4 는 M1 과 M2 를 기다린다. M5 는 마지막이다.

### M1 — 라이브 발행 1회

`rarn publish` 는 `--dry-run` 으로만 돌았다. 0.1.1 을 내면서 다른 선행조건은 전부 닫혔고,
실행이 남은 것은 이것 하나다.

**열린 결정 하나:** 무엇을 발행할지. 첫 발행이 그 스코프를 **영구히** 차지하고, 발행된 버전은
지울 수 없다(CLAUDE.md, 라이브 확인). 스코프 이름을 오타내면 그 오타가 스코프가 된다.
시작하기 전에 정해야 한다.

### M2 — 별칭 충돌 정책 확정

`assertNoAliasCollisions` 는 realm 세 개를 합쳐 **프로젝트 전역**으로 유일성을 강제한다.
R2 는 이것을 *"레이아웃이 요구하는 것보다 엄격하고 문서에 없다"* 고 적었다.

측정([test/benchmark](test/benchmark/README.md)): 인기 패키지 506개 중 **40개 별칭이 둘 이상의
패키지를 가리킨다.** `React` 는 `jsdotlua`·`haedrix`·`core-packages` 에 다 있다. 실제
프로젝트는 의존성 20~30개쯤에서 밟는다.

정할 것: 전역인가 realm 별인가 트리별인가, 그리고 부딪혔을 때 하드 에러인가 자동 구분인가.

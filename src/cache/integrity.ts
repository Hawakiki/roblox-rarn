...
/**
 * Verifies downloaded bytes against a recorded digest.
 *
 ...
 */
export function verifyIntegrity(bytes: Uint8Array, expected: string, subject: string): void {
  if (!isIntegrityString(expected)) {
    throw new RarnError({
  ...
  throw new RarnError({
    ...
    where: subject,
    detail: [`  expected: ${expected}`, `  actual:   ${actual}`].join('\n'),
